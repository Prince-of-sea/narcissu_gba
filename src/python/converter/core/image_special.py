    ...
    with Image.open(chara_0013b_path) as img:
        ...
        with Image.open(nsa_extract_path) as img_ex:
            ...
            with Image.open(filter_image_path) as img_alpha:
                ...
                # 切り出し
                img_ex = img_ex.crop((352, 236, 454, 279))

                # 縮小
                img_ex = img_ex.resize((66, 26))

                # 新画像にはりつけ
                img_new2.paste(img_ex, (87, 50))
...
def convert_IMG026(nsa_extract_path: Path, temppng_path: Path, cfg: AppConfig):
    ...
    # フィルター画像のパス
    filter_image_path = cfg.image_filter_dir / Path('filter_026_1.bin')
    ...
    with Image.open(sora_ame03_path) as img:
        ...
        with Image.open(nsa_extract_path) as img_ex:
            ...
            with Image.open(filter_image_path) as img_alpha:
                ...
                # 切り出し
                img_ex = img_ex.crop((291, 248, 527, 267))
...
def convert_IMG027(nsa_extract_path: Path, temppng_path: Path, cfg: AppConfig):
    ...
    # フィルター画像のパス
    filter_image_path = cfg.image_filter_dir / Path('filter_027_1.bin')
    ...
    with Image.open(sora07_path) as img:
        ...
        with Image.open(nsa_extract_path) as img_ex:
            ...
            with Image.open(filter_image_path) as img_alpha:
                ...
                # 切り出し
                img_ex = img_ex.crop((309, 242, 487, 260))

                # 縮小
                img_ex = img_ex.resize((120, 12))

                # 新画像にはりつけ
                img_new2.paste(img_ex, (60, 57))
